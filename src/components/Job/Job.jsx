...
const Job = ({ singleJob }) => {
  ...
  return (
    <div className="border border-[#E8E8E8] p-10 rounded-md">
      ...
      <Link to={`/job/${id}`} onClick={handleOnclick} className="btn bg-gradient-to-r from-[#7E90FE] to-[#9873FF] py-2 px-5 h-full mt-4"><span className="text-white font-semibold text-lg">View Details</span></Link>
      </div>
    </div>
  );
};
