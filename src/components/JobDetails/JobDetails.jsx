...
import { getStoredJobApplication, saveAllData, saveJobApplication } from "../../utilitis/LocalStorage/localStorage";
...
//  local storage
  const handleData = () =>{
    saveJobApplication(idx)
    saveAllData(jobDetails)
    const data = getStoredJobApplication()
    console.log(data)
  }
  return (
    <div>
      <div
        className="h-72 bg-[#7E90FE]/[.10] flex justify-center items-center bg-no-repeat bg-contain"
      ...
      <div className="container mx-auto my-24 grid grid-cols-4 gap-2">
        ...
        <div>
          <div>
            <button onClick={()=>handleData()} className="btn bg-gradient-to-r from-[#7E90FE] to-[#9873FF] py-3 px-5 h-full text-white">
              Apply Now
            </button>
          </div>
        </div>
